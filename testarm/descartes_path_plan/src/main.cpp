 ...
 */
void addFreeSpaceSegment(std::vector<geometry_msgs::Pose> waypoints, moveit::planning_interface::MoveGroupInterface& group, robot_trajectory::RobotTrajectory& overall_robot_traj);
 ...
 */
void addProcessSegment(TrajectoryVec points, descartes_planner::DensePlanner& planner, moveit::planning_interface::MoveGroupInterface& group, descartes_core::RobotModelPtr model, robot_trajectory::RobotTrajectory& overall_robot_traj, ros::NodeHandle& nh);
...
int main(int argc, char** argv)
{
  ...
  // TODO Fermi has an issue of hardcoded HD15. Avoid in Hamilton
  const std::string group_name = "HD15";

  // Name of frame in which you are expressing poses. Typically "world_frame" or "base_link".
  const std::string world_frame = "base_link";

  // tool center point frame (name of link associated with tool)
  const std::string tcp_frame = "Link_G";
  ...
  robot_trajectory::RobotTrajectory overall_robot_traj(group.getCurrentState()->getRobotModel(), "HD15"); //TODO group.getRobotModel
  ...
  for (unsigned int i = 0; i <= 10; ++i)
  {
    ...
    pose = Eigen::Translation3d(0.25, 0.25, 1 - 0.005*i); //CHECK with same z(0)=1.4 and z(0)=1.4-0.05
...
bool executeTrajectory(const trajectory_msgs::JointTrajectory& trajectory)
{
  ...
  actionlib::SimpleActionClient<control_msgs::FollowJointTrajectoryAction> ac("move_group", true);
...
void addProcessSegment(TrajectoryVec points, descartes_planner::DensePlanner& planner, moveit::planning_interface::MoveGroupInterface& group, descartes_core::RobotModelPtr model, robot_trajectory::RobotTrajectory& overall_robot_traj, ros::NodeHandle& nh)
{
  ...
  //Append planned path
  robot_trajectory::RobotTrajectory descartes_robot_traj(group.getCurrentState()->getRobotModel(), "HD15"); //TODO group.getRobotModel does the same thing. 
